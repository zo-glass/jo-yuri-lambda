...
class Base(ABC):
    ...
    AUXILIARY_KEYS = ["ttl", "status", "createdBy", "createdAt", "updatedBy", "updatedAt", "deletedBy", "deletedAt", "id"]
    CLOUDFRONT_DOMAIN = "cdn.zo.glass"
    BUCKET_NAME = "jo-yuri"
    ...
    def get_handler(self, queryStringParameters={}, full=False):
        ...
        if not full:
            args["filterExpression"] = Attr("status").eq("ready")
        ...
        data["pageInfo"]["totalResults"] = self.db.getCount(self.PARTITION, full=full)
    ...
    def get_by_id_handler(self, id, full=False):
        ...
        if not full and item.get("status") != "ready":
            return 404, {"errorDescription": "Item not found"}
    ...
    def post_handler(self, data, user, ttl=None):
        ...
        src = data.get("src", "")
        if self.CLOUDFRONT_DOMAIN in src:
            item["status"] = self.db.getStatus(self.PARTITION, src.split("/")[-1].split(".")[0])
        else:
            item["status"] = "ready"
    ...
    def delete_handler(self, id, queryStringParameters={}):
        ...
        try:
            item = None
            if force:
                item = self.db.force_delete_handler(self.PARTITION, id)
            else:
                item = self.db.delete_handler(self.PARTITION, id)

            if item and self.CLOUDFRONT_DOMAIN in item.get("src", ""):
                self.s3_del_handler(item["src"], force=force)
        except ValueError as e:
            return 404, {"errorDescription": str(e)}
        except Exception as e:
            return 500, {"errorDescription": "Internal Server Error"}
        ...
        if not full:
            for item in items:
                if item.get("status") == "ready":
                    if "SK" in item and "id" not in item:
            ...
            for item in items:
                ...
                key = "items" if status == "ready" else (status or "other")
    ...
    def s3_del_handler(self, src, force=False):
        key = unquote(urlparse(src).path.lstrip("/"))
        ...
        self.s3.copy_object(
            ...
            Tagging="ttl1d=true" if force else "ttl30d=true"
        )
