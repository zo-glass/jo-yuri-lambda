...
class YouTube():
    ...
    def start(self):
        ttl = int(time.time()) + 48 * 60 * 60
        items = self.fetchVideos()
        for item in items:
            self.video.post_handler(item, "Lambda", ttl=ttl)
    ...
    def fetchVideos(self):
        ...
        while (True):
            ...
            for item in response["items"]:
                ...
                video["id"] = item["snippet"]["publishedAt"]
                video["title"] = item["snippet"]["title"]
                video["alt"] = item["snippet"]["title"]
                video["href"] = f"https://www.youtube.com/watch?v={item["snippet"]['resourceId']['videoId']}"
                video["src"] = f"https://img.youtube.com/vi/{item["snippet"]['resourceId']['videoId']}/maxresdefault.jpg"
                #video["src"] = f"https://img.youtube.com/vi/{item["snippet"]['resourceId']['videoId']}/hqdefault.jpg"
                video["youtubeId"] = item["snippet"]['resourceId']['videoId']
