...
class Schedule(Base):
    ...
    def get_handler(self, queryStringParameters={}, full=False):
        ...
        pageToken = self.tokenToLek(queryStringParameters.get("pageToken"))
        res = self.db.get_by_range_handler(self.PARTITION, start, end, pageToken=pageToken)
    ...
    def post_handler(self, data, user, ttl=None):
        ...
        item["id"] = f"{data['start']}_{lastId.split('#')[-1]}"
        item["status"] = "ready"
